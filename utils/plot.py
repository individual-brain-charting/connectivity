        ...
        for sub in task_subjects:
            ...
            for cov in cov_estimators:
                for measure in measures:
                    ...
                    connectivity = np.nanmean(connectivity, axis=0)
                    # skip if connectivity is all nan
                    if np.isnan(connectivity).all():
                        print(
                            f"Skipping {task} {sub} {cov} {measure} due to all NaN connectivity"
                        )
                        continue
