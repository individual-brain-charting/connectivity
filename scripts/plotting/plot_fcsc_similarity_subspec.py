...
filtered_similarity_data = similarity_data[
    similarity_data["comparison"].isin(comparisons_to_keep)
]

# only keep these measures
cov_measures = [
    "Graphical-Lasso partial correlation",
    "Ledoit-Wolf correlation",
    "Unregularized correlation",
]
# comnbine cov_estimators and measures columns in df
filtered_similarity_data = filtered_similarity_data[
    filtered_similarity_data["measure"].isin(cov_measures)
]

for centering in similarity_data["centering"].unique():
    data = filtered_similarity_data[
        filtered_similarity_data["centering"] == centering
    ]
    data["corrected_p_value_t"] = false_discovery_control(data["p_value_t"])
    data["corrected_p_value_mwu"] = false_discovery_control(
        data["p_value_mwu"]
    )
    for measure in cov_measures:
        df = data[
            (data["centering"] == centering)
            & (data["measure"] == f"{measure}")
        ]
        for test in ["t", "mwu"]:
            d = {
                "Comparison": [],
            ...
            for _, row in df.iterrows():
                ...
                d["Comparison"].extend(
                    ...
                    + [f"{row['comparison']} Across Subject"] * len(cross_sub)
                )
                d["Task vs. SC"].extend(
                    [f"{row['comparison']}"] * (len(same_sub) + len(cross_sub))
                )
                d["FC measure"].extend(
                    [row["measure"]] * (len(same_sub) + len(cross_sub))
                )
                d["Similarity"].extend(same_sub + cross_sub)
                p_values[row["comparison"]] = row[f"corrected_p_value_{test}"]
            d = pd.DataFrame(d)
            hue_order = [
            ...
            plt.title(f"{measure}", loc="right", x=-1, y=1.05)
            plot_file = os.path.join(
                output_dir,
                f"{measure}_{centering}_{test}_box.svg",
            )
            plot_file2 = os.path.join(
                output_dir,
                f"{measure}_{centering}_{test}_box.png",
            )
